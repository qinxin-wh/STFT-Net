...
# network related
from lib.models.stark import build_starks, build_stft
from lib.models.stark import build_stark_lightning_x_trt
# forward propagation related
from lib.train.actors import STARKSActor, STFTActor
...
def run(settings):
    ...
    elif settings.script_name == "stark_st1" or settings.script_name == "stft":
        net = build_stft(cfg)
    ...
    # devs = ["cuda:{}".format(i) for i in range(2)]
    # dev = [0,1]
    ...
    elif settings.script_name == "stft":
        objective = {'cls': BCEWithLogitsLoss(), 'giou': giou_loss, 'l1': l1_loss}
        loss_weight = {'cls': 1.0, 'reg': 1.0, 'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT}
        actor = STFTActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings)
    elif settings.script_name == "stark_lightning_X_trt":
        objective = {'giou': giou_loss, 'l1': l1_loss}
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT}
    ...
    # train process
    if settings.script_name in ["stft"]:
