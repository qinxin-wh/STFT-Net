...
class STFT(BaseTracker):
    def __init__(self, params, dataset_name, net_path):
        super(STFT, self).__init__(params)
        device = torch.device('cuda:0')
        network = build_starkst(params.cfg)
        # network.load_state_dict(torch.load(net_path, map_location=lambda storage, loc: storage.cuda(0))['net'], strict=True)
        network.load_state_dict(torch.load(net_path)['net'], strict=True)
