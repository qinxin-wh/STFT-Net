...
def generalized_box_iou(boxes1, boxes2):
    ...
    for j in range(len(idx)):
        pred = boxes1[idx[j], :]
        gt = boxes2[idx[j], :]
        pred_box.append(pred)
        gt_box.append(gt)

    if len(idx) != 0:
        boxes1_new = torch.stack(pred_box, dim=0)
        boxes2_new = torch.stack(gt_box, dim=0)

    assert (boxes1_new[:, 2:] >= boxes1_new[:, :2]).all()
    assert (boxes2_new[:, 2:] >= boxes2_new[:, :2]).all()
    print(count)
