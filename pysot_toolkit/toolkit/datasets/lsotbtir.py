...
class LSOTBVideo(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect, img_names, gt_rect, load_img=False):
        super(LSOTBVideo, self).__init__(name, root, video_dir,
    ...
    def load_tracker(self, path, tracker_names=None, store=True):
        ...
        for name in tracker_names:
            a = os.path.join(path, 'LSOTBTIR', name, self.name+'.txt')
            traj_files = glob(os.path.join(path, 'LSOTBTIR', name, self.name+'.txt'))
...
class LSOTBDataset(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        super(LSOTBDataset, self).__init__(name, dataset_root)
        ...
        for video in pbar:
            ...
            #                               load_img=load_img)
            self.videos[video] = LSOTBVideo(video,
                                             dataset_root,
                                             meta_data[video]['video_dir'],
                                             meta_data[video]['init_rect'],
                                             meta_data[video]['img_names'],
